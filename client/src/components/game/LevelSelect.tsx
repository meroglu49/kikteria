...
const stagger = {
  animate: { transition: { staggerChildren: 0.015, delayChildren: 0.1 } },
};
const popIn = {
  ...
  return (
    <div className="min-h-screen overflow-y-auto bg-background scanlines crt-vignette scrollable-screen">
      ...
      <button
        ...
        className="fixed top-6 left-6 block-panel px-4 py-2 hover:border-primary transition-colors flex items-center gap-2 z-20 active:scale-95"
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto px-6 py-20 flex flex-col items-center gap-6">
        <div className="text-center">
          <h1 
            className="text-3xl md:text-4xl font-display animate-slide-down"
            style={{
              color: '#22F2A2',
          ...
          <p className="font-ui text-xl text-secondary text-glow tracking-widest mt-2 animate-slide-up">
            Complete levels to unlock more!
          </p>
        </div>

        <motion.div 
          className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 w-full pb-8"
          variants={stagger}
          initial="initial"
          animate="animate"
        >
          {LEVELS.map((level, index) => {
            ...
            return (
              <motion.button
                ...
                disabled={!isUnlocked}
                className={`
                  relative block-panel p-2 sm:p-3 flex flex-col items-center gap-1 rounded-lg active:scale-95
                  ${isUnlocked 
        ...
        {!user && (
          <div className="block-panel px-6 py-3 text-center animate-slide-up">
