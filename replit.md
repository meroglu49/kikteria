...
- `client/src/lib/__tests__/layout-validation.test.ts` - **Layout boundary tests for mobile/desktop devices**
- `server/__tests__/storage.test.ts` - Player profiles, leaderboard, level progress, admin methods

### Layout Validation Tests
Automated tests verify UI elements stay within screen bounds across devices:
- **Device emulation**: iPhone SE, iPhone 14 Pro Max, Pixel 5, iPad, Desktop
- **Boundary checks**: Canvas, sidebar, top bar, queue bar positions
- **Overflow prevention**: No horizontal/vertical scroll issues
- **Touch targets**: Minimum sizes for mobile interaction

**Minimalistic Bar Dimensions (optimized for mobile play area):**
- Sidebar: w-11 (44px) mobile, sm:w-20 (80px) desktop
- Top bar: h-9 (36px) mobile, sm:h-12 (48px) desktop
- Queue bar: h-10 (40px) mobile, sm:h-12 (48px) desktop

Run layout tests: `npx vitest run client/src/lib/__tests__/layout-validation.test.ts`
...
### UI Redesign (v10.1)
- **Performance**: HUD split into 14 memoized sub-components with Zustand selectors (ScoreDisplay, TimerDisplay, BombButton, etc.) — eliminates unnecessary re-renders during gameplay
- **FigureQueue**: Template lookup cache, individual Zustand selectors
- **Animations**: AnimatePresence page transitions in App.tsx, CSS-based animations (slide-up, slide-down, scale-in, pop-in, shake, banner-in, glow-ring), staggered list entries
- **Button responsiveness**: Replaced framer-motion whileHover/whileTap on all menu buttons with CSS `active:scale-*` for instant feedback, reduced transition durations from 0.15s to 0.1s
- **Tutorial**: Fully disabled (imports commented out)
- **Tab-nums**: Timer uses `tabular-nums` to prevent layout shift
